package com.github.khangzxrr.web.rest;

import com.github.khangzxrr.repository.RequestBidRepository;
import com.github.khangzxrr.service.RequestBidService;
import com.github.khangzxrr.service.dto.RequestBidDTO;
import com.github.khangzxrr.web.rest.errors.BadRequestAlertException;
import java.net.URI;
import java.net.URISyntaxException;
import java.util.List;
import java.util.Objects;
import java.util.Optional;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;
import tech.jhipster.web.util.HeaderUtil;
import tech.jhipster.web.util.ResponseUtil;

/**
 * REST controller for managing {@link com.github.khangzxrr.domain.RequestBid}.
 */
//@RestController
//@RequestMapping("/api/request-bids")
public class RequestBidResource {

    private final Logger log = LoggerFactory.getLogger(RequestBidResource.class);

    private static final String ENTITY_NAME = "requestBid";

    @Value("${jhipster.clientApp.name}")
    private String applicationName;

    private final RequestBidService requestBidService;

    private final RequestBidRepository requestBidRepository;

    public RequestBidResource(RequestBidService requestBidService, RequestBidRepository requestBidRepository) {
        this.requestBidService = requestBidService;
        this.requestBidRepository = requestBidRepository;
    }

    /**
     * {@code POST  /request-bids} : Create a new requestBid.
     *
     * @param requestBidDTO the requestBidDTO to create.
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new requestBidDTO, or with status {@code 400 (Bad Request)} if the requestBid has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("")
    public ResponseEntity<RequestBidDTO> createRequestBid(@RequestBody RequestBidDTO requestBidDTO) throws URISyntaxException {
        log.debug("REST request to save RequestBid : {}", requestBidDTO);
        if (requestBidDTO.getId() != null) {
            throw new BadRequestAlertException("A new requestBid cannot already have an ID", ENTITY_NAME, "idexists");
        }
        RequestBidDTO result = requestBidService.save(requestBidDTO);
        return ResponseEntity
            .created(new URI("/api/request-bids/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /request-bids/:id} : Updates an existing requestBid.
     *
     * @param id the id of the requestBidDTO to save.
     * @param requestBidDTO the requestBidDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated requestBidDTO,
     * or with status {@code 400 (Bad Request)} if the requestBidDTO is not valid,
     * or with status {@code 500 (Internal Server Error)} if the requestBidDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/{id}")
    public ResponseEntity<RequestBidDTO> updateRequestBid(
        @PathVariable(value = "id", required = false) final Long id,
        @RequestBody RequestBidDTO requestBidDTO
    ) throws URISyntaxException {
        log.debug("REST request to update RequestBid : {}, {}", id, requestBidDTO);
        if (requestBidDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, requestBidDTO.getId())) {
            throw new BadRequestAlertException("Invalid ID", ENTITY_NAME, "idinvalid");
        }

        if (!requestBidRepository.existsById(id)) {
            throw new BadRequestAlertException("Entity not found", ENTITY_NAME, "idnotfound");
        }

        RequestBidDTO result = requestBidService.update(requestBidDTO);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, requestBidDTO.getId().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /request-bids/:id} : Partial updates given fields of an existing requestBid, field will ignore if it is null
     *
     * @param id the id of the requestBidDTO to save.
     * @param requestBidDTO the requestBidDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated requestBidDTO,
     * or with status {@code 400 (Bad Request)} if the requestBidDTO is not valid,
     * or with status {@code 404 (Not Found)} if the requestBidDTO is not found,
     * or with status {@code 500 (Internal Server Error)} if the requestBidDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<RequestBidDTO> partialUpdateRequestBid(
        @PathVariable(value = "id", required = false) final Long id,
        @RequestBody RequestBidDTO requestBidDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update RequestBid partially : {}, {}", id, requestBidDTO);
        if (requestBidDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, requestBidDTO.getId())) {
            throw new BadRequestAlertException("Invalid ID", ENTITY_NAME, "idinvalid");
        }

        if (!requestBidRepository.existsById(id)) {
            throw new BadRequestAlertException("Entity not found", ENTITY_NAME, "idnotfound");
        }

        Optional<RequestBidDTO> result = requestBidService.partialUpdate(requestBidDTO);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, requestBidDTO.getId().toString())
        );
    }

    /**
     * {@code GET  /request-bids} : get all the requestBids.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of requestBids in body.
     */
    @GetMapping("")
    public List<RequestBidDTO> getAllRequestBids() {
        log.debug("REST request to get all RequestBids");
        return requestBidService.findAll();
    }

    /**
     * {@code GET  /request-bids/:id} : get the "id" requestBid.
     *
     * @param id the id of the requestBidDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the requestBidDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/{id}")
    public ResponseEntity<RequestBidDTO> getRequestBid(@PathVariable("id") Long id) {
        log.debug("REST request to get RequestBid : {}", id);
        Optional<RequestBidDTO> requestBidDTO = requestBidService.findOne(id);
        return ResponseUtil.wrapOrNotFound(requestBidDTO);
    }

    /**
     * {@code DELETE  /request-bids/:id} : delete the "id" requestBid.
     *
     * @param id the id of the requestBidDTO to delete.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteRequestBid(@PathVariable("id") Long id) {
        log.debug("REST request to delete RequestBid : {}", id);
        requestBidService.delete(id);
        return ResponseEntity
            .noContent()
            .headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, id.toString()))
            .build();
    }
}
