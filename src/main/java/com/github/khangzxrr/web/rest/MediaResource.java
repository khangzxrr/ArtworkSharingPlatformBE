...
/**
 * REST controller for managing {@link com.github.khangzxrr.domain.Media}.
 */
@RestController
...
public class MediaResource {
    ...
    public List<MediaDTO> getAllMedia(@RequestParam(name = "filter", required = false) String filter) {
        if ("artworkasset-is-null".equals(filter)) {
            log.debug("REST request to get all Medias where artworkAsset is null");
            return mediaService.findAllWhereArtworkAssetIsNull();
        }

        if ("certificate-is-null".equals(filter)) {
            log.debug("REST request to get all Medias where certificate is null");
            return mediaService.findAllWhereCertificateIsNull();
        }
